...
class Send {
    ...
    static success(res: Response, data: any, message = "success", extra: Record<string, any> = {}) {
      res.status(200).json({
        ok: true,
        message,
        data,
        ...extra, // 👈 merge fields เพิ่มเติม
      });
    }
  ...
  static error(res: Response, data: any, message = "error", statusCode = 500) {
